...
export default function Input({ label, value, setValue, placeholder, Icon }) {
  let inputStyle = `border-slate-200 border-2 focus:border-slate-100 ${
    Icon && "pl-8"
  } rounded-md py-2 bg-white text-gray-500 font-semibold`;
  let iconStyle = "absolute gap-10 w-6 h-6 ml-3";

  return (
    <div>
      {label && (
        <label className="text-md font-semibold text-gray-500">{label}</label>
      )}
